...
func TestGetSymbolVerifiesBeforeTruncation(t *testing.T) {
	...
	if err := store.ReplaceRepoIndex("local", "verify", "local", "", map[string]string{"main.go": "hash"}, map[string]string{"main.go": "go"}, []parser.Symbol{sym}); err != nil { t.Fatal(err) }
	if err := store.SaveContentFile("local", "verify", "main.go", content); err != nil { t.Fatal(err) }
	deps := &Deps{Store: store}
	value, err := execGetSymbol(deps, GetSymbolArgs{Repo: "local/verify", SymbolID: sym.ID, Verify: true, MaxLength: 12})
	if err != nil { t.Fatal(err) }
	result := value.(map[string]any)
	if result["content_verified"] != true { t.Fatalf("complete source should verify even when response is truncated: %#v", result) }
	if source == result["source"] { t.Fatal("expected truncated source") }
}
...
func TestToTextResultUsesCompactValidJSON(t *testing.T) {
	...
	if text != `{"ok":true,"items":["a","b"]}` { t.Fatalf("expected compact JSON, got %q", text) }
}
